defmodule Spiral do
  ...
  defp spiral(0, {_, _, acc}), do: acc

  defp spiral(run_length, {{x, y} = initial, %{increment: increment, next: next}, acc} = _acc) do
    {
      increment({x, y}, run_length, increment),
      @directions[next],
      acc ++ side(initial, run_length, increment)
    }
  end

  defp side({x, y}, run_length, increment) do
    for i <- 1..run_length, do: increment({x, y}, i, increment)
  end
